...
from .apihelpers import _rootNodeOrRaise, _documentOrRaise
from .apihelpers import _removeUnusedNamespaceDeclarations
from .apihelpers import _findChildForwards, _nextElement
from .apihelpers import _removeNode, _removeSiblings, _replaceNodeByChildren
...
def _strip_attributes(c_node, matcher):
...
def strip_elements(tree_or_element, *tag_names, **kwargs):
...
def _strip_elements(doc, c_node, matcher, with_tail):
    for c_node in FOR_EACH_ELEMENT_FROM(c_node, c_node, 1):
        if c_node.type == tree.XML_ELEMENT_NODE:
...
def _strip_tags(doc, c_node, matcher):
    for c_node in FOR_EACH_ELEMENT_FROM(c_node, c_node, 1):
        if c_node.type == tree.XML_ELEMENT_NODE:
            # we run through the children here to prevent any problems
            # with the tree iteration which would occur if we unlinked the
